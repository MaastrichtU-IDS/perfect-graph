...
export default (props: SongProps) => {
  ...
  const play = React.useCallback(async () => {
    if (!soundObjectRef.current) {
      const {sound: soundObject, status} = await Audio.Sound.createAsync({uri: url}, {})
  ...
  return (
    <Graph.View style={styles.container}>
      ...
      <Graph.Touchable onPress={() => (state.isPlaying ? pause() : play())}>
        <Graph.View style={styles.innerContainer}>
          <Graph.Image source={{uri: state.isPlaying ? PAUSE_IMAGE_URL : PLAY_IMAGE_URL}} style={styles.icon} />
        </Graph.View>
      </Graph.Touchable>
    </Graph.View>
  )
