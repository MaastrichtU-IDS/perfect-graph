...
import {getSelectedItemByElement, getUndoEvents, getSelectedElementInfo, vectorMidpoint, getHitAreaCenter} from '@utils'
import {EDITOR_MODE, EVENT, DEFAULT_EDGE_CONFIG, DEFAULT_NODE_CONFIG} from '@constants'
...
export const useController = (
  ...
  const controllerConfig: UseControllerData = React.useMemo<UseControllerData>(
    () => R.mergeDeepAll([DEFAULT_CONTROLLER_CONFIG, useControllerData]) as UseControllerData,
    [] // useControllerData
  )
  const eventHistory = React.useMemo(
    () =>
      createHistory({
        onEvent: event => {
          const {actions} = event
  ...
  const onEvent = React.useCallback(
    (eventInfo: EventInfo) => {
      ...
      const element = elementId ? graphEditor.cy.$id(`${elementId}`) : null
        ...
        try {
          if (!avoidHistoryRecording) {
            const {
              ...
              events: undoEvents
              // @ts-ignore
            } = getUndoEvents([eventInfo], {graphEditor, controllerState: state})
            if (addHistory) {
              eventHistory.add({
                do: [
                  {
          ...
          if (draft.actionBar?.eventRecording && type !== EVENT.TOGGLE_RECORD_EVENTS && !avoidEventRecording) {
          ...
          const {item: eventRelatedItem} = (element && getSelectedItemByElement(element, draft)) ?? {}
          ...
          switch (type) {
            ...
            case EVENT.CHANGE_DATA_VALUE:
              targetDataList[index].value[payload.valueIndex] = getValueByType(dataItem.type!, payload.value)
              break
            ...
            case EVENT.DATA_VALUE_UP: {
              ...
              const changeIndex = (payload.valueIndex === 0 ? length - 1 : payload.valueIndex - 1) % length
            ...
            case EVENT.IMPORT_DATA: {
              ...
              const {eventHistory: eventHistoryData} = (payload.value ?? {}) as unknown as {
            ...
            case EVENT.SET_POSITIONS_IMPERATIVELY: {
              const {positions, oldLayout} = payload
              type PositionItem = {
                elementId: string
                position: Position
              }
            ...
            case EVENT.SELECT_CLUSTER: {
              ...
              draft.selectedElementIds = R.concatAll(selectedClusters!.map(cluster => cluster.ids))
              break
            }
            ...
            case EVENT.DELETE_CLUSTER_ELEMENT: {
              const {clusterId, itemIds = []} = payload
              const selectedCluster = draft.graphConfig?.clusters?.find(cluster => cluster.id === clusterId)
              selectedCluster!.ids = selectedCluster!.ids.filter(id => !itemIds.includes(id))
              break
            }
            case EVENT.CREATE_CLUSTER: {
              const {items = []} = payload
              draft.graphConfig!.clusters = (draft.graphConfig?.clusters?.concat(items) ?? []).map(cluster => ({
                ...cluster,
            ...
            case EVENT.REORDER_HISTORY_ITEM: {
              ...
              eventHistory.reorder(fromIndex, toIndex)
              break
            }
            ...
            case EVENT.REORDER_CLUSTER: {
              const {fromIndex, toIndex} = payload
              draft.graphConfig!.clusters = R.reorder(fromIndex, toIndex, draft.graphConfig!.clusters!)
              break
            }
            ...
            case EVENT.REORDER_PLAYLIST: {
              const {fromIndex, toIndex} = payload
              draft.playlists = R.reorder(fromIndex, toIndex, draft.playlists!)
              break
            }
            ...
            case EVENT.PREFERENCES_FORM_CLEAR: {
              const {value, preferenceId} = payload
              // draft.modals.ElementSettings.isOpen = true
              switch (preferenceId) {
                case 'NodeView':
  ...
  return [
    ...
    {
      ...state,
      ...(!_graphEditorRef ? {ref: localGraphEditorRef} : {}),
      eventHistory: {
        currentIndex: eventHistory.record.currentIndex - 1,
        events: R.unnest(R.map(item => item.do, eventHistory.record.items)),
        undoEvents: R.unnest(R.map(item => item.undo, eventHistory.record.items))
      },
      onEvent
