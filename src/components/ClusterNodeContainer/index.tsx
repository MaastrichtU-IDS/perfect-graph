...
import {RenderClusterNode, NodeConfig, GraphRef, Cluster} from '@type'
import {calculateObjectBoundsWithoutChildren, calculateVisibilityByContext} from '@utils'
...
const ClusterNodeContainerElement = (
  ...
  const {item, graphID, children, config = {} as NodeConfig, graphRef} = props
  ...
  React.useEffect(() => {
    const container = containerRef.current!
    context.boundingBox = calculateObjectBoundsWithoutChildren(container)
  })
  const theme = useTheme()
  const visible = calculateVisibilityByContext(element) && (item.visible ?? true)
  const opacity = context.settings.filtered ? 1 : config.filter?.settings?.opacity ?? 0.2
  return (
    <Container
      ref={containerRef}
      x={x}
 ...
 * operations.
 */
export const ClusterNodeContainer = wrapComponent<ClusterNodeContainerProps>(ClusterNodeContainerElement, {
  isForwardRef: true
})
