...
import {RenderEdge, EdgeConfig, DrawLine, EdgeElement, NodeElement, GraphRef} from '@type'
...
export const calculateVectorInfo = (source: NodeElement, to: NodeElement) => {
  ...
  const midpointPosition = vectorMidpoint(fromPosition, toPosition)
...
const EdgeContainerElement = (props: EdgeContainerProps, __: React.ForwardedRef<EdgeContainerType>) => {
  const {
    item,
    // item: _item,
    graphID,
  ...
  const drawLineCallback = React.useCallback(
    ({
      ...
      const {
        view: {lineType, fill, width, alpha}
      } = config
      return drawLine({
        item,
        element,
  ...
  const onPositionChange = React.useCallback(
    ({element}) => {
      ...
      const vectorInfo = calculateVectorInfo(element.source(), element.target())
      containerRef.current!.x =
        vectorInfo.midpointPosition.x + edgeGroupInfo.sortedIndex * vectorInfo.undirectedNormVector.x * DEFAULT_DISTANCE
      containerRef.current!.y =
        vectorInfo.midpointPosition.y + edgeGroupInfo.sortedIndex * vectorInfo.undirectedNormVector.y * DEFAULT_DISTANCE
      drawLineCallback({
        cy,
        element,
        edgeGroupInfo,
  ...
  const targetElement = element.target()
  const edgeGroupInfo = calculateEdgeGroupInfo(element)
  const vectorInfo = calculateVectorInfo(sourceElement, targetElement)
  React.useEffect(() => {
    drawLineCallback({
      cy,
      element,
  ...
  const opacity = filtered ? 1 : config?.filter?.settings?.opacity ?? 0.2
  ...
  return (
    <>
      <Container
        ...
        alpha={opacity}
        visible={visible}
        x={midpointPosition.x + edgeGroupInfo.sortedIndex * undirectedNormVector.x * DEFAULT_DISTANCE}
        y={midpointPosition.y + edgeGroupInfo.sortedIndex * undirectedNormVector.y * DEFAULT_DISTANCE}
 ...
 * operations.
 */
export const EdgeContainer = wrapComponent<EdgeContainerProps>(EdgeContainerElement, {
  isForwardRef: true
})
