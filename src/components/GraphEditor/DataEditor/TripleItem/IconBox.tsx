...
export const IconBox = (props: IconBoxProps) => {
  const {onExtend, extended, onEvent, isLocalLabel, isGlobalLabelFirst} = props
  return (
    <Box
      style={{
        alignItems: 'center',
    ...
    >
      ...
      >
        ...
        >
          <Icon name={isLocalLabel ? 'bookmarks' : 'bookmark_border'} style={{fontSize: ICON_SIZE}} />
        </IconButton>
        {isGlobalLabelFirst && <Divider style={{backgroundColor: 'black', width: ICON_SIZE - 2}} />}
      </Box>
      <Box
        style={{
          display: 'flex',
      ...
      >
        <IconButton onClick={onExtend}>
          <Icon style={{fontSize: ICON_SIZE}} name={extended ? 'arrow_drop_up_rounded' : 'arrow_drop_down_rounded'} />
        </IconButton>
        {extended && (
          <IconButton onClick={() => onEvent({type: EVENT.DELETE_DATA})}>
            <Icon
              name="delete_rounded"
              style={{
                fontSize: ICON_SIZE
