...
} & Partial<Pick<FormProps<any>, 'onChange' | 'onSubmit' | 'formData' | 'uiSchema' | 'children'>>
...
const SECTIONS = ['layout', 'history', 'playlist', 'clusters']
...
const SettingsBarElement = (props: SettingsBarProps) => {
  ...
  const [{onEvent, eventHistory, clusters, playlists, layout}] = useGraphEditor(editor => ({
      ...
      >
        ...
        >
          {forms.map((form, index) => {
            ...
            const {isOpen} = formSection
            return (
              <React.Fragment key={title}>
                <Collapsible isOpen={isOpen}>
                  {() => (
                    <>
                      <CollapsibleTitle onClick={() => formSection.onChange(!isOpen)}>{title}</CollapsibleTitle>
                      {isOpen && (
            ...
            <>
              ...
              >
                <EventHistoryTable
                  ...
                  onCreatePlaylistClick={selectedEventIds =>
                    updateState(draft => {
          ...
          {playlists && (
            <PlaylistTable
              ...
              onCreatePlaylist={playlistWithoutEvents => {
                const playlistEvents = state.selectedEventIds
                  .map(eventId => eventHistory!.events.find(event => event.id === eventId)!)
          ...
          {clusters && (
            <>
              ...
              >
                <ClusterTable {...state.sections[SECTION_NAMES.clusters]} createClusterForm={createClusterForm} />
              </View>
              <Divider style={{marginTop: 5, marginBottom: 5}} />
            </>
          )}
