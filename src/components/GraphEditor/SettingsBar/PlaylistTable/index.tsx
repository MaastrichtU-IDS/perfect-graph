...
export type EventHistoryTableProps = CollapsibleSectionCommon & {
  ...
  onCreatePlaylist: (param: {id: string; name: string}) => void
}

export const PlaylistTable = (props: EventHistoryTableProps) => {
  const {createPlaylistDialog, onCreatePlaylist, isOpen, onChange} = props
  const [{onEvent, playlists = []}] = useGraphEditor(editor => ({
        ...
        {() => (
          <>
            ...
            >
              ...
              >
                <CollapsibleTitle onClick={() => onChange(!isOpen)}>Playlists</CollapsibleTitle>
              </View>
            </View>
            {isOpen && hasSelected && (
              <Card
              ...
              >
                <ListItem>
                  <Checkbox
                    ...
                    checked={
                      !R.isEmpty(state.selectedPlaylistIds) && state.selectedPlaylistIds.length === playlists.length
                    }
                    onChange={e => {
            ...
            {isOpen && (
              <CollapsibleContainer>
                {playlists.length === 0 && <Typography variant="subtitle2">Let's create a playlist.</Typography>}
                <List dense>
                  <SortableList
                    onReorder={result =>
                      onEvent({
                    ...
                    data={playlists}
                    renderItem={({provided, item: playlist}) => {
                          ...
                          <AccordionSummary aria-controls="panel1a-content">
                            ...
                            >
                              ...
                              >
                                <Checkbox
                                  onClick={e => e.stopPropagation()}
                                  checked={hasSelected && state.selectedPlaylistIds.includes(playlist.id)}
                                  onChange={e => {
                                    const {checked} = e.target
                                    updateState(draft => {
                                      if (checked) {
