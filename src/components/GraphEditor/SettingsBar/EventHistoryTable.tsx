...
import {Card, Checkbox, IconButton, List, ListItem, ListItemAvatar, ListItemText, Typography} from '@mui/material'
import {View, wrapComponent} from 'colay-ui'
...
const EventHistoryTableElement = (props: EventHistoryTableProps) => {
  const {onCreatePlaylistClick, isOpen, onChange} = props
  const [state, updateState] = useImmer({
    selectedEventIds: [] as string[]
  })
  const hasSelected = state.selectedEventIds.length > 0
  const [{onEvent, eventHistory}] = useGraphEditor(editor => ({
        ...
        {() => (
          <>
            ...
            >
              ...
              >
                <CollapsibleTitle onClick={() => onChange(!isOpen)}>History</CollapsibleTitle>
              </View>
              <View
                style={{
            ...
            {isOpen && hasSelected && (
              ...
              >
                <ListItem>
                  <Checkbox
                    checked={
                      !R.isEmpty(state.selectedEventIds) &&
            ...
            {isOpen && (
              <CollapsibleContainer>
                {eventHistory!.events.length === 0 && <Typography>Let's do a few things.</Typography>}
                <List dense>
                  <SortableList
                    onReorder={result => {
                    ...
                    renderItem={({provided, item: event, index}) => {
                      const {length} = eventHistory!.events
                      return (
                        <ListItem
                          key={event.id}
