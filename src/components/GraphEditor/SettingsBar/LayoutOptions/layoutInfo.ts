export const LAYOUT_INFO = {
  ...
  euler: {
    ...
    content: 'The euler layout is a traditional force-directed algorithm. It is fairly fast. However, cola tends to give better results, and fcose tends to be both faster and higher quality.',
  },
  cise: {
    title: 'cise',
    image: 'https://raw.githubusercontent.com/sabaturgay/assets/main/gifs/ciseLayout.gif',
    content: 'The cise layout organises the nodes into clusters. Each cluster is represented as a circle. Similar to the avsdf layout, this layout tries to avoid edge overlap within each circle. The clusters are positioned using a force-directed algorithm. If you have well defined clusters in your graph, the cise layout is a good fit.',
  },
  cola: {
    title: 'cola',
    image: 'https://raw.githubusercontent.com/sabaturgay/assets/main/gifs/colaLayout.gif',
    content: 'The cola layout differentiates itself by allowing for setting constraints on top of the traditional force-directed physics simulation. If you want to set your own rules for how some nodes are organised, then cola is a good fit. This layout also tends to create smooth transitions in node position from one iteration to the next, so it is well suited to successive or infinite runs of the layout.',
  },
  dagre: {
    title: 'dagre',
    image: 'https://raw.githubusercontent.com/sabaturgay/assets/main/gifs/dagreLayout.gif',
    content: 'The dagre layout is a traditional hierarchical layout. It looks like how people typically draw binary trees on a whiteboard: It has a nice inverted V shape from one level to the next. This should be one of the first layouts that you try if your graph is a DAG.',
  },
  avsdf: {
    title: 'avsdf',
    image: 'https://raw.githubusercontent.com/sabaturgay/assets/main/gifs/avsdfLayout.gif',
    content: 'The avsdf layout is another circle layout. Whereas the circle layout is useful when you want to order the nodes yourself, the avsdf layout is useful when you want to automatically order the nodes to try to avoid edge overlap.',
  },
  spread: {
    title: 'spread',
    image: 'https://raw.githubusercontent.com/sabaturgay/assets/main/gifs/spreadLayout.gif',
    content: 'The spread layout is a two-phased layout. First, it runs cose and then it spreads out the graph to fill out the viewport as much as possible. The spread layout is a good fit when you want the graph to be as spread out as possible while still having an overall force-directed organisation.',
  },
  klay: {
    title: 'klay',
    image: 'https://raw.githubusercontent.com/sabaturgay/assets/main/gifs/klayLayout.gif',
    content: 'The klay layout is a traditional hierarchical layout. It is the predecessor to the layered algorithm in elk. You may want to use klay for its smaller file size, if the layered algorithm of elk does not differ significantly for your dataset.',
  },
}
