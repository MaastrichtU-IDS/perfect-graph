...
export const ConnectedElements = () => {
  const [
    ...
    const {selectedElement, onEvent} = editor
    return {
  ...
  if (isNode) {
    itemList = selectedElement.connectedEdges().toArray().map(getItemFromElement)
    connectedNodeitemList = selectedElement
      .neighborhood()
      .toArray()
      .filter(el => el.isNode())
      .map(getItemFromElement)
  } else {
    itemList = [selectedElement.source(), selectedElement.target()].map(getItemFromElement)
  }
  return (
    <>
