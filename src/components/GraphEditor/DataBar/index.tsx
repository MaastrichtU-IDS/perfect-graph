...
import {useAnimation} from 'colay-ui'
...
export const DataBar = (props: DataBarProps) => {
  const {editable = false, isOpen = false, sort = -1, header: HeaderComponent, footer: FooterComponent} = props

  const [{item, onEvent, networkStatistics, globalLabel, localLabel, isGlobalLabelFirst}] = useGraphEditor(editor => {
    const {selectedElement, selectedItem, label, localDataRef, networkStatistics} = editor
  ...
  return (
    ...
    >
      ...
      >
              ...
              {({isOpen, onToggle}) => (
                ...
                isEdge && <EdgeElementSummary element={selectedElement} />
                        ...
                        {editable && (
                          ...
                          >
                            <Button onClick={() => setState({...state, isEditing: !state.isEditing})}>
        ...
        <ResizeDivider onMouseDown={onMouseDown} isRight={false} />
      </Paper>
    </Box>
  )
}
