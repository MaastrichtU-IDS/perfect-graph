...
export const createHistory = (options: CreateHistoryOptions) => {
  ...
  const history = {
    ...
    add: (item: HistoryItemOptionalId) => {
      ...
      record.items = R.slice(0, currentIndex)(items)
      record.items.push({
        id: R.uuid(),
        ...item
      })
    ...
    undoAll: () =>
      R.when(R.allPass([R.isNotNil, () => record.items.length > record.currentIndex]), () => {
        const {items} = record
        const relatedItems = R.slice(0, record.currentIndex + 1)(items)
        const undoList = R.unnest(R.map(item => item.undo, relatedItems)) as UndoActions[]
        const doList = R.unnest(R.map(item => item.do, relatedItems)) as DoActions[]
        onEvent({
          item: {
            id: R.uuid,
            do: doList,
