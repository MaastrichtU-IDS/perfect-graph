...
import {EdgeContext, EdgeConfig, EdgeElement, EdgeData} from '@type'
...
export const useEdge = (props: Props): Result => {
  const {onPositionChange, graphID, config = {} as EdgeConfig, item} = props
  ...
  React.useEffect(() => {
    const nodeDataUpdated = () => {
      ...
      const newNodeFiltered = sourceContext.settings.filtered && targetContext.settings.filtered
